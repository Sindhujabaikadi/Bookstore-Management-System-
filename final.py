...
def login_user(user_id):
    ...
    if authenticate_user(user_id, st.session_state['user_type']):
        ...
        #st.success(f"Welcome {st.session_state['user_type']} : {st.session_state['user_name']}!")
    ...
    for book in books_data:
        ...
        with cols[2]:
            if st.button(f"Delete Book {book['BookID']}", key=str(book['BookID'])):
                books_collection.delete_one({"BookID": book['BookID']})  # Delete book from MongoDB
...
def add_book():
    ...
    if st.button("Add Book", key="add_book_button"):
        ...
        if not all([book_title, book_author, book_price, book_genre, book_year, book_publisher]):
